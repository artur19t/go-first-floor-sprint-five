...
func (r Running) TrainingInfo() InfoMessage {
	...
	//Возможно, можно было как то по другому сделать, не копируя почти тот же кусок кода?
	return InfoMessage{
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	if w.Height > 0 {
		coef := (CaloriesWeightMultiplier*w.Weight + (math.Pow((w.meanSpeed()*KmHInMsec), 2)/(w.Height/CmInM))*CaloriesSpeedHeightMultiplier*w.Weight)
